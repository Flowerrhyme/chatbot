...
conn = sqlite3.connect('./QA_data/QA_extend.db', check_same_thread=False)
...
def match(input_question):
    ...
    for tag in question: #按照每个tag，循环构造查询语句
        keyword = "'%" + tag + "%'"
        result = cursor.execute("select * from QA2 where tag like " + keyword)
        for row in result:
    ...
    cursor.execute("select * from QA2 where id= " + str(res_id))
    res = cursor.fetchone()
